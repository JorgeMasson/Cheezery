...
class ProductosActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var img_titulo: ImageView = findViewById(R.id.img_titulo) as ImageView
        val tipo = intent.getStringExtra("tipo")

        if (tipo == "colddrinks") {
            agregaColdDrinks()
            img_titulo.setImageResource(R.drawable.colddrinks)
        } else if (tipo == "hotdrinks") {
            agregaHotDrinks()
            img_titulo.setImageResource(R.drawable.hotdrinks)
        }else if (tipo == "sweets") {
            agregaSweets()
            img_titulo.setImageResource(R.drawable.sweets)
        }else if (tipo == "salties") {
            agregaSalties()
            img_titulo.setImageResource(R.drawable.salties)
        }
    ...
    fun agregaColdDrinks() {
        products.add(Product("Caramel Frap", R.drawable.caramelfrap, "Caramel syrup meets coffee, milk and ice and whipped cream and buttery caramel sauce layer the love on top.", 6.0))
